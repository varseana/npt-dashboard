...
export default function SelfView({ email, aliasOverride }: { email: string; aliasOverride: string | null }) {
  ...
  if (everReported === false) {
    return (
      <div style={card}>
        <h2 style={{ margin: "0 0 8px", fontSize: 32 }}>No NPT data for "{alias}"</h2>
        <p style={{ color: palette.textDim, fontSize: 24, margin: 0, lineHeight: 1.5 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <span style={{ color: palette.textDim, fontSize: 23 }}>Signed in as {alias}</span>
      </div>
      ...
      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 10, fontSize: 26 }}>Breakdown (what counts as NPT)</div>
        ...
        {plannedSec == null && (
          <div style={{ marginTop: 12, color: palette.textDim, fontSize: 23 }}>
...
function Stat({ label, value, extra }: { label: string; value: string; extra?: React.ReactNode }) {
  return (
    <div style={{ ...card, padding: "16px 18px" }}>
      <div style={{ color: palette.textDim, fontSize: 21, marginBottom: 6 }}>{label}</div>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 39, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{value}</span>
        {extra}
      </div>
    </div>
  );
...
const select: React.CSSProperties = {
  ...
  borderRadius: 8, padding: "8px 10px", fontSize: 24,
};
