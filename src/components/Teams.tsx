...
export default function Teams({ refreshKey }: { refreshKey: number }) {
  ...
  if (loading) return <div style={{ maxWidth: 760 }}><TableSkeleton rows={3} /></div>;

  return (
    <div style={{ maxWidth: 760 }}>
      {msg && <div style={{ marginBottom: 12, color: msg.startsWith("Error") ? palette.bad : palette.warn, fontSize: 23 }}>{msg}</div>}
      ...
      <div style={{ background: palette.panelAlt, border: `1px solid ${palette.border}`, borderRadius: 8, padding: "14px 16px", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <label style={{ fontSize: 23, color: palette.textDim, display: "flex", alignItems: "center", gap: 6 }}>
            Target NPT %
        ...
        <div style={{ color: palette.textDim, fontSize: 21, marginTop: 8 }}>
          ...
          return (
            <div key={t.id} style={{ border: `1px solid ${palette.border}`, borderRadius: 8, marginBottom: 14, overflow: "hidden" }}>
              ...
              <div style={{ padding: "10px 14px", borderTop: `1px solid ${palette.border}`, background: palette.bg }}>
                <div style={{ fontSize: 21, color: palette.textDim, marginBottom: 8, fontWeight: 600 }}>Enrollment codes</div>
                {tCodes.length === 0 && <div style={{ fontSize: 23, color: palette.textDim, marginBottom: 8 }}>No codes yet.</div>}
                {tCodes.map((c) => (
                  <div key={c.code} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                    <code style={{ fontSize: 23, fontWeight: 700 }}>{c.code}</code>
                    <span style={{ fontSize: 21, padding: "1px 8px", borderRadius: 6, color: c.active ? palette.ok : palette.textDim, background: c.active ? palette.okBg : palette.panelAlt }}>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <div style={{ padding: "12px 14px", color: palette.textDim, fontSize: 23, border: `1px solid ${palette.border}`, borderRadius: 8 }}>{children}</div>;
}

const input: React.CSSProperties = {
  background: palette.panel, color: palette.text, border: `1px solid ${palette.border}`,
  borderRadius: 8, padding: "8px 10px", fontSize: 24, boxSizing: "border-box",
};
const btn: React.CSSProperties = {
  background: palette.accent, color: "#fff", border: "none", borderRadius: 8,
  padding: "8px 14px", fontSize: 23, cursor: "pointer", fontWeight: 600,
};
const btnGhost: React.CSSProperties = {
  background: palette.panel, color: palette.text, border: `1px solid ${palette.border}`,
  borderRadius: 8, padding: "6px 12px", fontSize: 23, cursor: "pointer",
};
