...
export default function Planned({ team }: { team: Team }) {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <button onClick={save} disabled={saving} style={{ marginTop: 16, background: palette.accent, color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>
        {saving ? "Saving..." : "Save planned"}
      </button>
      {msg && <div style={{ marginTop: 12, color: msg.startsWith("Error") ? palette.bad : palette.ok, fontSize: 13 }}>{msg}</div>}
    </div>
  );
}
