  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ background: palette.panelAlt, border: `1px solid ${palette.border}`, borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
        <div style={{ fontSize: 14, color: palette.textDim, marginBottom: 6 }}>Team default ({team.name}) :: {scopeLabel}</div>
      ...
      <button onClick={save} disabled={saving} style={{ marginTop: 16, background: palette.accent, color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", fontSize: 16, cursor: "pointer", fontWeight: 600 }}>
        {saving ? "Saving..." : "Save planned"}
      </button>
      {msg && <div style={{ marginTop: 12, color: msg.startsWith("Error") ? palette.bad : palette.ok, fontSize: 15 }}>{msg}</div>}
    </div>
  );
}
